...
# Twitch chat emotes / messages that fit any stream context
_CHAT_MESSAGES = [
...
class TwitchRunner(BasePlatformRunner):
    ...
    def _try_like(self, driver) -> bool:
        # Twitch doesn't have a traditional "like" — we treat clip/VOD thumbs-up
        try:
    ...
    def _try_comment(self, driver) -> bool:
        """Post a random emote/message into Twitch chat."""
        try:
            chat_input = None
            for sel in [
    ...
    def _dwell_extra(self, driver, tick_seconds):
        """Occasionally scroll the chat panel while dwelling."""
    ...
    def inspect_platform_state(self, driver, profile_id, session_id="", selected_target=None):
        ...
        # Ad detection (never skip — observe only for IP quality)
        ads_detected = None
        try:
            ad_selectors = [
